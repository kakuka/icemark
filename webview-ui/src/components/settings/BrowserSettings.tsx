...
type BrowserSettingsProps = HTMLAttributes<HTMLDivElement> & {
	...
	preLoginBrowserEnabled?: boolean
	...
	return (
		<div {...props}>
			...
			<Section>
				...
				<div>
					<VSCodeCheckbox
						checked={preLoginBrowserEnabled}
						onChange={(e: any) => setCachedStateField("preLoginBrowserEnabled", e.target.checked)}>
						<span className="font-medium">{t("settings:browser.preLogin.label")}</span>
					</VSCodeCheckbox>
					<div className="text-vscode-descriptionForeground text-sm mt-1">
						{t("settings:browser.preLogin.description")}
					</div>
					
					{preLoginBrowserEnabled && (
						<div className="mt-2 flex flex-col gap-2">
							<div className="flex gap-2">
								<VSCodeButton onClick={() => vscode.postMessage({ type: "launchPreLoginBrowser" })}>
									{t("settings:browser.preLogin.launchButton")}
								</VSCodeButton>
								<VSCodeButton 
									appearance="secondary"
									onClick={() => vscode.postMessage({ type: "clearBrowserLoginData" })}
									style={{ backgroundColor: '#dc2626', borderColor: '#dc2626', color: 'white' }}>
									{t("settings:browser.preLogin.clearButton")}
								</VSCodeButton>
							</div>
							<div className="text-vscode-descriptionForeground text-xs mt-1">
								{t("settings:browser.preLogin.instructions")}
							</div>
						</div>
					)}
				</div>
