...
export const ChatRowContent = ({
	...
	if (tool) {
		...
		switch (tool.tool) {
					...
					<>
						<div style={headerStyle}>
							...
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask"
									? tool.isOutsideWorkspace
										? t("chat:fileOperations.wantsToSaveOutsideWorkspace")
										: t("chat:fileOperations.wantsToSave")
									: t("chat:fileOperations.didSave")}
							</span>
						</div>
						<div
							style={{
						...
						{(() => {
							...
							const renderItems = (arr: any[], level: number): JSX.Element[] => {
								...
								for (let idx = 0; idx < arr.length; idx++) {
									...
									if (Array.isArray(it?.children) && it.children.length) {
										out.push(...renderItems(it.children, level + 1))
									}
								}
								return out
							}
							return (
								<div
									...
									<ul style={{ margin: 0, paddingLeft: 0 }}>{renderItems(items, 0)}</ul>
								</div>
							)
						})()}
					</>
					...
					<>
						<div style={headerStyle}>
							...
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask"
									? tool.isOutsideWorkspace
										? t("chat:initJobWorkDir.wantsToCreateOutsideWorkspace")
										: t("chat:initJobWorkDir.wantsToCreate")
									: t("chat:initJobWorkDir.didCreate")}
							</span>
						</div>
						<div
							style={{
							...
							<div
								...
								<span
									...
									{removeLeadingNonAlphanumeric(`${tool.path || ""}/${tool.folder || ""}`) + "\u200E"}
									{tool.reason}
								</span>
								<div style={{ flexGrow: 1 }}></div>
								<span
				...
				return (
					<>
						<div style={headerStyle}>
							...
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask"
									? tool?.saveToFileFullPath
										? t("chat:fileOperations.wantsToFetchUrlAndSave")
										: t("chat:fileOperations.wantsToFetchUrl")
									: tool?.saveToFileFullPath
										? t("chat:fileOperations.didFetchUrlAndSave")
										: t("chat:fileOperations.didFetchUrl")}
							</span>
						</div>
						<div
							style={{
							...
							<div
								...
								onClick={() => {
									if (tool?.url) {
										vscode.postMessage({ type: "openExternal", text: tool.url })
									}
								}}>
								<span
									style={{
							...
							{tool?.saveToFileFullPath && (
								<div
									...
									onClick={() => {
										if (tool?.path || tool?.saveToFileFullPath) {
											vscode.postMessage({
												type: "openFile",
												text: tool?.path || tool?.saveToFileFullPath,
											})
										}
									}}>
									{tool?.path?.startsWith(".") && <span>.</span>}
									<span
										...
										{removeLeadingNonAlphanumeric(tool?.path || tool?.saveToFileFullPath || "") +
											"\u200E"}
									</span>
									<div style={{ flexGrow: 1 }}></div>
									<span
			...
			case "webSearch":
				const searchPlatformNames: Record<string, string> = {
					general: t("chat:webSearch.platforms.general", "通用搜索引擎"),
					xiaohongshu: t("chat:webSearch.platforms.xiaohongshu", "小红书"),
					zhihu: t("chat:webSearch.platforms.zhihu", "知乎"),
					weibo: t("chat:webSearch.platforms.weibo", "微博"),
				}
				return (
					<>
						<div style={headerStyle}>
							...
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask"
									? t("chat:fileOperations.wantsToSearchWeb")
									: t("chat:fileOperations.didSearchWeb")}
							</span>
						</div>
						<div
							style={{
							...
							<div
								...
								<span style={{ marginRight: "8px", fontSize: "12px", opacity: 0.8 }}>
									{t("chat:fileOperations.keywords")}:
								</span>
								<div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
									{tool?.keywordList?.map((keyword, index) => (
										<span
											key={index}
											style={{
												backgroundColor: "var(--vscode-badge-background)",
								...
								<div
									...
									{tool?.searchOn && (
										<span style={{ marginRight: "16px" }}>
											{t("chat:webSearch.platform")}:{" "}
											{searchPlatformNames[tool.searchOn] || tool.searchOn}
										</span>
									)}
									{tool?.pageLimit && (
										<span>
											{t("chat:webSearch.pageLimit")}: {tool.pageLimit}
										</span>
									)}
								</div>
							)}
				...
				return (
					<>
						<div style={headerStyle}>
							{toolIcon("tools")}
							<span style={{ fontWeight: "bold" }}>
								{message.type === "ask" ? t("chat:prototype.wantsToUse") : t("chat:prototype.didUse")}
							</span>
						</div>
						<div
							style={{
							...
							<div
								...
								{tool?.path && (
									<div style={{ marginBottom: "4px" }}>
										<span style={{ fontWeight: "500" }}>📁 {t("chat:prototype.path")}:</span>{" "}
										<span
											style={{
								...
								{tool?.folder && (
									<div style={{ marginBottom: "4px" }}>
										<span style={{ fontWeight: "500" }}>📂 {t("chat:prototype.folder")}:</span>{" "}
								...
								{tool?.action && (
									<div style={{ marginBottom: "4px" }}>
										<span style={{ fontWeight: "500" }}>🔧 {t("chat:prototype.action")}:</span>{" "}
										<span style={{ fontWeight: "500", color: "var(--vscode-editor-foreground)" }}>
											{tool.action}
										</span>
									</div>
								...
								{tool?.content && (
									<div style={{ fontSize: "12px", opacity: 0.8 }}>详情：{tool.content}</div>
								)}
							</div>
						</div>
					</>
