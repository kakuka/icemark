...
import RooHero from "@/components/welcome/IcemarkHero"
...
const ChatViewComponent: React.ForwardRefRenderFunction<ChatViewRef, ChatViewProps> = (
	...
	useEffect(() => {
		if (task) {
			setWorkspaceExpanded(false) // 进入任务页时折叠
		}
		// 回到引导页时不修改状态，保持之前的状态
	}, [task])
	...
	const handleSecondaryButtonClick = useCallback(
		(text?: string, images?: string[]) => {
			...
			if (isStreaming) {
				console.log("handleSecondaryButtonClick, isStreaming is true, cancelTask")
		...
		<div className={isHidden ? "hidden" : "fixed top-0 left-0 right-0 bottom-0 flex flex-col overflow-hidden"}>
			...
			{task ? (
				...
				<div className="flex-1 min-h-0 flex flex-col">
					...
					<div
						className={`flex-1 overflow-y-auto w-full flex flex-col gap-4 m-auto ${isExpanded && tasks.length > 0 ? "mt-0" : ""} px-3.5 min-[370px]:px-10 pt-5 transition-all duration-300`}>
						<RooHero />
						...
						<p className="ext-vscode-editor-foreground leading-tight font-vscode text-center">
							<Trans
								...
								components={{
									DocsLink: (
										<a href="https://github.com/kakuka/icemark" target="_blank" rel="noopener noreferrer">
											the docs
										</a>
									),
								}}
