...
const BaseConfigSchema = z.object({
	...
	transportType: z.enum(["sse", "streamableHttp"]).optional(), // 新增字段
})

// Custom error messages for better user feedback
const typeErrorMessage = "Server type must be either 'stdio', 'sse', or 'streamableHttp'"
const stdioFieldsErrorMessage =
	"For 'stdio' type servers, you must provide a 'command' field and can optionally include 'args' and 'env'"
const sseFieldsErrorMessage =
	"For 'sse' type servers, you must provide a 'url' field and can optionally include 'headers'"
const mixedFieldsErrorMessage =
	"Cannot mix 'stdio' with other field types. For 'stdio' use 'command', 'args', and 'env'. For 'sse' or 'streamableHttp' use 'url' and 'headers'"
const missingFieldsErrorMessage = "Server configuration must include either 'command' (for stdio) or 'url' (for sse/streamableHttp)"
const streamableHttpFieldsErrorMessage = "For 'streamableHttp' type servers, you must provide a 'url' field"
...
const createServerTypeSchema = () => {
	return z.union([
		...
		// StreamableHTTP config (has url field and type streamableHttp)
		BaseConfigSchema.extend({
...
export class McpHub {
	...
	private validateServerConfig(config: any, serverName?: string): z.infer<typeof ServerConfigSchema> {
		...
		const hasStreamableHttpFields = config.transportType === "streamableHttp" || config.type === "streamableHttp"
		const hasSseFields = config.transportType === "sse" || config.type === "sse" || config.url !== undefined

		// Check for mixed fields
		if (hasStdioFields && (hasSseFields || hasStreamableHttpFields)) {
		...
		if (!config.type) {
			if (hasStdioFields) {
				...
				config.type = "sse"
			} else if (hasStreamableHttpFields) {
				config.type = "streamableHttp"
			} else {
				throw new Error(missingFieldsErrorMessage)
			}
		} else if (config.type !== "stdio" && config.type !== "sse" && config.type !== "streamableHttp") {
		...
		if (config.type === "sse" && !hasSseFields) {
			console.error("in config validateServerConfig sseFieldsErrorMessage", sseFieldsErrorMessage);
  ...
  "mcpServers": {
	...
	private async connectToServer(
		...
		try {
			...
			// 处理配置，同时支持transportType字段
			const rawConfig = config as any;
			console.log(`Connection attempt for ${name}, config:`, JSON.stringify(rawConfig, null, 2));

			// 改进streamableHttp检测，支持更多情况
			const hasStreamableHttpTransport = 
				(typeof rawConfig.transportType === 'string' && 
				rawConfig.transportType === 'streamableHttp' &&
				typeof rawConfig.url === 'string') || 
				(rawConfig.type === 'streamableHttp' &&
				typeof rawConfig.url === 'string');

			console.log(`Using StreamableHTTP: ${hasStreamableHttpTransport}`);

			if (hasStreamableHttpTransport) {
				try {
					// 使用StreamableHTTP传输 - 修复构造函数调用
					console.log(`Using StreamableHTTP transport for server "${name}" at ${rawConfig.url}`);
					...
					// 设置错误处理
					transport.onerror = async (error: Error | string | unknown) => {
						console.error(`StreamableHTTP transport error for "${name}":`, error);
					...
					console.error(`Failed to initialize StreamableHTTP transport for "${name}":`, error);
					throw error;
				}
				...
				const reconnectingEventSourceOptions = {
					max_retry_time: 50000, // Maximum retry time in milliseconds
	...
	async updateServerConnections(
		...
		for (const [name, config] of Object.entries(newServers)) {
			...
			try {
				...
				console.log("!!! invalid configuration for MCP server", name, error);
				this.showErrorMessage(`1 Invalid configuration for MCP server "${name}"`, error)
				continue
			}
		...
		if (config) {
			...
			try {
				...
				try {
					...
					this.showErrorMessage(`2 Invalid configuration for MCP server "${serverName}"`, validationError)
				}
			} catch (error) {
