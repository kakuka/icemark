...
import { FileReadRequest } from "../../services/file/FileService"
...
export async function readFileTool(
	...
	try {
		if (block.partial) {
			...
			// 构建FileService请求
			const fileReadRequest: FileReadRequest = {
				filePath: relPath,
				startLine: startLine ? startLine + 1 : undefined, // 转换回1-based
				endLine: endLine ? endLine + 1 : undefined,
				maxLines: maxReadFileLine,
				fullRead: isFullRead,
				cwd: cline.cwd
			}

			// 使用FileService读取文件
			const result = await cline.fileService.readFile(fileReadRequest)
			
			const content = result.content
			const totalLines = result.totalLines
			const isFileTruncated = result.isFileTruncated
			const sourceCodeDef = result.sourceCodeDef || ""
