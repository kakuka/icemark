...
import { SearchEngineService } from "../services/browser/SearchEngineService"
import { listFiles } from "../services/glob/list-files"
import { BrowserSession } from "../services/browser/BrowserSession"
import { PrototypeService } from "../services/prototype/PrototypeService"
...
import { saveResourceTool } from "./tools/saveResourceTool"
import { initJobWorkDirTool } from "./tools/initJobWorkDirTool"
...
export class Cline extends EventEmitter<ClineEvents> {
	...
	constructor({
		...
		this.searchEngineService = new SearchEngineService(provider.context)
		this.browserSession = new BrowserSession(provider.context)
		this.prototypeService = new PrototypeService(provider.context, this.cwd)
		this.fileService = new FileService(this.rooIgnoreController)
		...
		if (startTask) {
			if (task || images) {
				...
				this.providerRef.deref()?.log("task is " + task)
	...
	private async getSavedClineMessages(): Promise<ClineMessage[]> {
		// console.log("getSavedClineMessages", this.taskId)
	...
	async resumePausedTask(lastMessage: string) {
		...
		this.providerRef
			.deref()
			?.log(
				"resumePausedTask with lastMessage: " +
					lastMessage +
					" , task id is " +
					this.taskId +
					" , parentTask id is " +
	...
	async *attemptApiRequest(previousApiReqIndex: number, retryAttempt: number = 0): ApiStream {
		...
		//this.providerRef.deref()?.log("systemPrompt is \n\n" + systemPrompt)
			...
			case "tool_use":
				const toolDescription = (): string => {
					switch (block.name) {
						...
						case "save_resource":
							return `[${block.name} for '${block.params.uri}' to '${block.params.folder}/${block.params.filename}']`
						case "init_job_workdir":
							return `[${block.name} creating '${block.params.folder}' in '${block.params.path}']`
						case "url_content_fetch":
							return `[${block.name} fetching content from '${block.params.url}'${block.params.save_to_file_full_path ? ` and saving to '${block.params.save_to_file_full_path}'` : ""}]`
						case "web_search":
				...
				switch (block.name) {
					...
					case "new_task":
						...
						break
					case "save_resource":
						await saveResourceTool(this, block, askApproval, handleError, pushToolResult, removeClosingTag)
						break
					case "init_job_workdir":
						await initJobWorkDirTool(
							this,
							block,
							askApproval,
							handleError,
					...
					case "url_content_fetch":
						...
						break
					case "web_search":
						await webSearchTool(this, block, askApproval, handleError, pushToolResult, removeClosingTag)
						break
					case "prototype":
		...
		if (this.consecutiveMistakeCount >= this.consecutiveMistakeLimit) {
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "Icemark uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.7 Sonnet for its advanced agentic coding capabilities.",
			)
		...
		try {
			...
			try {
				for await (const chunk of stream) {
					...
					switch (chunk.type) {
						case "reasoning":
							...
							this.say("reasoning", reasoningMessage, undefined, true)
							break
						case "usage":
				...
				if (!this.abandoned) {
					...
					if (history) {
						this.providerRef.deref()?.log("after error, initClineWithHistoryItem ")
						this.providerRef.deref()?.log("this.taskId is " + this.taskId)
						this.providerRef.deref()?.log("this.parentTask id is " + this.parentTask?.taskId)
						this.providerRef.deref()?.log("this.rootTask id is " + this.rootTask?.taskId)
						this.providerRef.deref()?.log("history.historyItem is " + history.historyItem)
						await this.providerRef.deref()?.initClineWithHistoryItem({
							...history.historyItem, // 保留所有原始历史数据
							rootTask: this.rootTask, // 重新建立根任务链接
							parentTask: this.parentTask, // 重新建立父任务链接
						})
