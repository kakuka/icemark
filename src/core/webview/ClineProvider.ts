...
export class ClineProvider extends EventEmitter<ClineProviderEvents> implements vscode.WebviewViewProvider {
	public static readonly sideBarId = "icemark.SidebarProvider" // used in package.json as the view's id. This value cannot be changed due to how vscode caches views based on their id, and updating the id would break existing instances of the extension.
	public static readonly tabPanelId = "icemark.TabPanelProvider"
	private static activeInstances: Set<ClineProvider> = new Set()
	private static initializationLock: Promise<void> = Promise.resolve()
	...
	public readonly latestAnnouncementId = "icemark-1-2-9" // Update for v1.0 announcement
	...
	async addClineToStack(cline: Cline) {
		this.log("start addClineToStack")
		this.log("cline.taskId is " + cline.taskId + " , cline.instanceId is " + cline.instanceId)
	...
	async removeClineFromStack() {
		this.log("start removeClineFromStack")
		this.log("current stack is " + this.clineStack.map((c) => `${c.taskId}.${c.instanceId}`).join(", "))
		...
		if (cline) {
			...
			try {
				...
				this.log("removeClineFromStack error is " + e.message)
				this.log(
	...
	*/
	...
	async getStateToPostToWebview() {
		...
		const allowedCommands = vscode.workspace.getConfiguration("icemark").get<string[]>("allowedCommands") || []
		const cwd = this.cwd
	...
	async getState() {
		...
		return {
			...
			preLoginBrowserEnabled: (() => {
				const value = stateValues.preLoginBrowserEnabled
				if (value === undefined) {
					// 如果全局状态中没有值，设置默认值并写入
					this.contextProxy.setValue("preLoginBrowserEnabled", true)
					return true
				}
				return value
			})(),
