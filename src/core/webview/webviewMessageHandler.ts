...
export const webviewMessageHandler = async (provider: ClineProvider, message: WebviewMessage) => {
	...
	switch (message.type) {
		...
		case "openImage":
			...
			break
		case "openFile":
			console.log("openFile", message.text!, message.values as { create?: boolean; content?: string })
			openFile(message.text!, message.values as { create?: boolean; content?: string })
			break
		case "openFolder":
			// 使用VSCode命令打开文件夹选择对话框
			vscode.commands.executeCommand("workbench.action.files.openFolder")
			break
		case "revealInExplorer":
			// 在系统文件管理器中显示指定路径
			if (message.text) {
				vscode.commands.executeCommand("revealInExplorer", vscode.Uri.file(message.text))
			}
			break
		case "openExternal":
			if (message.text) {
		...
		case "setHistoryPreviewCollapsed": // Add the new case handler
			...
			break
		case "preLoginBrowserEnabled":
			await updateGlobalState("preLoginBrowserEnabled", message.bool ?? false)
			await provider.postStateToWebview()
			break
		case "launchPreLoginBrowser":
			try {
				const { PreLoginBrowser } = await import("../../services/browser/PreLoginBrowser")
				const preLoginBrowser = new PreLoginBrowser(provider.context)
				await preLoginBrowser.launch()
				provider.log("Pre-login browser launched successfully")
			} catch (error) {
				provider.log(`Failed to launch pre-login browser: ${error.message}`)
			}
			break
		case "clearBrowserLoginData":
			try {
				const { PreLoginBrowser } = await import("../../services/browser/PreLoginBrowser")
				const preLoginBrowser = new PreLoginBrowser(provider.context)
				await preLoginBrowser.clearLoginData()
				provider.log("Browser login data cleared successfully")
				vscode.window.showInformationMessage("已清除所有浏览器登录信息")
			} catch (error) {
				provider.log(`Failed to clear browser login data: ${error.message}`)
				vscode.window.showErrorMessage("清除登录信息失败：" + error.message)
			}
			break
		case "prototype":
			try {
				...
				const prototypeService = new PrototypeService(provider.context, cwd)
				...
				if (action === "scan") {
					...
					// 处理 init 和 show 操作
					if (!filePath) {
						provider.log("No file path provided for prototype operation")
						return
					}
